...
Допустим, что чемпион игрока — это Yone. Он купил первым предметом BoRK. Его пассивный эффект наносит цели физический урон при каждой автоатаке. Этот урон равен 8% от текущего уровня здоровья цели. Пусть противника собрал 200 брони. Она уменьшает входящий физический урон на 66%. Это означает, что on-hit урон BoRK против такой цели уменьшается с 8% до 2.7%. Другими словами предмет перестаёт работать.

Чтобы решить проблему низкого урона по бронированной цели, игроки с BoRK дополняют его предметом на пробивание брони. Например, Lord Dominik's Regards — игнорирует 35% брони цели. Для нашего примера это означает, что броня противника уменьшится с 200 до 130. Если пересчитать на проценты, физический урон будет уменьшаться не на 66%, а на 56%. Тогда on-hit урон от BoRK вырастет с 2.7% до 3.5%. Таким образом Lord Dominik's Regards усилил купленный ранее BoRK.
...
Чемпион с BoRK может решить проблему танков с броней иначе. Обычно умения таких чемпионов имеют on-hit эффекты с разными типами урона. Примеры: Vayne W и Kog'Maw W. Они покупают вторым предметом Guinsoo's Rageblade. Благодаря ему, после каждых трёх автоатак все on-hit эффекты срабатывают ещё раз. Поэтому за три автоатаки BoRK будет наносить не `8% * 3 = 24%` урона от текущего здоровья цели, а `8% * 4 = 32%`. Аналогично увеличивается on-hit магический или true урон от умения чемпиона. Из-за этого вражеский танк не сможет собирать только броню. Ему придётся покупать предметы на сопротивлении магии или дополнительное здоровье. Таким образом Guinsoo's Rageblade усиливает и BoRK, и умения чемпиона.

Эффект от BoRK можно ещё больше усилить с помощью рун. AD Carry, которые его покупают, берут руну Lethal Tempo. Она увеличивает скорость атаки и on-hit урон BoRK срабатывает чаще.

Также BoRK хорошо комбинируется с руной Cut Down. Она наносит 8% дополнительного урона, если цель имеет более 60% здоровья. On-hit урон BoRK максимален, когда цель имеет полное здоровье. Cut Down усиливает его ещё больше. В результате игрок может быстро пробить любую цель до половины здоровья.

> Внимательно читайте эффекты рун и предметов, которые рекомендуются для вашего чемпиона. Подумайте, почему они работают и какая между ними синергия.

### 5.3.2 Классификация предметов

>>>R2

Разработчики делят все предметы на четыре уровня. Следующий список приводит их в порядке от самого низкого к самому высокому:

1. **Starter** (начальные) — покупаются на первой минуте и не входят в сборки предметов более высокого уровня.

2. **Basic** (базовые) — немного увеличивают только один параметр чемпиона или дают незначительный эффект. Входят в сборки предметов следующего уровня.

3. **Epic** (эпические) — увеличивают несколько параметров чемпиона и дают средний эффект. Входят в сборки предметов следующего уровня.

4. **Legendary** (легендарные) — значительно увеличивают несколько параметров чемпиона, а также дают сильный эффект.

Эта классификация указывает на порядок сборки предметов. Сначала игрок выбирает один из starter предметов. Как правило, они самые эффективные по золоту. Далее игрок покупает несколько basic предметов, которые собираются в один epic. Затем несколько epic предметов собираются в один legendary.

Порядок покупки предметов от basic к legendary объясняет игровую механику. Но он не отвечает на главный вопрос: какие именно legendary предметы следует собирать и в каком порядке?

У опытных игроков есть альтернативная классификация предметов. Она учитывает только легендарные предметы и определяет порядок их сборки. Они делятся на три категории:
...
### 5.3.3 Предметы для класса AD Carry
...
Начальные предметы дают самый сильный ранний power spike. Он складывается из высоких бонусных параметров предмета и его сильного эффекта. Конкретный выбор зависит от чемпиона, его рун и матчапа.

Список начальных предметов следующий:
...
### 5.3.4 Предметы для класса AP Carry
...
### 5.3.5 Предметы для класса Tank
...
### 5.3.6 Предметы для класса Assassin
...
### 5.3.7 Предметы для класса Bruiser
...
### 5.3.8 Предметы для класса Support

>>>W

### 5.3.9 Подготовка билдов
