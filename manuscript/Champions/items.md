...
Это означает, что чемпион получит не 100, а только 80 урона за счёт своей брони.

Теперь выполним расчёт в обратную сторону. Определим, сколько нужно исходного урона (pre-mitigation), чтобы убить чемпиона с 1000 здоровья. Если его броня равна нулю, то конечный урон будет равен исходному. Другими словами, 1000 исходного урона хватит, чтобы убить чемпиона с 1000 здоровья.

В нашем примере броня чемпиона равна 25. Чтобы его убить, нужно нанести 1000 конечного урона (post-mitigation). Этот урон нам известен. Вычислим по нему исходный урон (pre-mitigation). Для этого преобразуем формулу для расчёта конечного урона так:
{line-numbers: false, format: text}
```
...
Это означает, что 1250 урона убьёт чемпиона с 1000 здоровья и 25 брони. Полученное значение 1250 — это есть эффективное здоровье против физического урона.
...
Эффективное здоровье можно использовать для расчёта процентного уменьшения урона за счёт брони и сопротивления магии. Именно это отображает игра, если навести курсор мыши на количество брони вашего чемпиона.

Рассчитаем процентное уменьшение урона для нашего примера. Чемпион имеет 1000 здоровья и 25 брони. Его эффективное здоровье равно 1250. Тогда он будет получать следующий процент от входящего физического урона (pre-mitigation):
{line-numbers: false, format: text}
```
1000 / 1250 = 0.8
```
...
Если у чемпиона будет 200 брони и 1000 здоровья, его эффективное здоровье будет таким:
{line-numbers: false, format: text}
```
effective health = 1000 / (100 / (100 + 200)) = 3000
```

Тогда он будет получать такой процент от входящего физического урона:
{line-numbers: false, format: text}
```
1000 / 3000 = 0.33
```
