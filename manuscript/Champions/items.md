...
I> Учитывайте изменения баланса в следующих патчах, когда будете следовать советам из этого раздела.

Обзор предметов мы начнём с общих принципов по их выбору.

### 5.3.1 Выбор предметов

Разработчики меняют их характеристики почти в каждом патче. В начале очередного сезона они удаляют некоторые предметы из игры и добавляют новые. Поэтому информация по предметам быстро устаревает.
...
2. **Предметы должны синергировать (сочетаться) друг с другом**. Это усиливает очередной power spike чемпиона.

3. **Предметы должны синергировать с выбранными рунами**. Это также усиливает power spike.
