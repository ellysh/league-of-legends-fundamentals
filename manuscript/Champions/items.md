...
Обзор предметов мы начнём с общих рекомендаций по их выбору.

### 5.3.1 Выбор предметов

>>>R2

Есть несколько общих рекомендаций по выбору предметов. Они не зависят от текущего баланса, роли и класса чемпиона. Эти рекомендации следующие.

**Во-первых**, следующие два правила помогут выбрать между атакующим и защитным предметом:

1. Если игрок лидирует по убийствам или ресурсам, ему выгоднее купить защитный предмет. Его урона уже достаточно, чтобы убивать противников. С другой стороны, с защитным предметом противники не смогут его убить. Таким образом игрок ещё больше увеличит свой отрыв.

2. Если игрок отстаёт по убийствам или ресурсам, ему выгоднее купить атакующий предмет. Так он сократит своё отставание по урону и получит шанс убить противника.

Покупая защитный предмет, игрок минимизирует риск умереть в очередном командном сражении и отдать награду противнику (bounty). Это выгодно, потому что игрок уже сильнее оппонентов и ему нет смысла лишний раз рисковать.

Когда игрок отстаёт, его единственный вариант вернуться в игру — пойти на риск. Поэтому он должен купить атакующий предмет. Если ему повезёт, он нанесёт достаточно урона в командном сражении, чтобы сделать убийство и получить награду.
...
**В-третьих**, усиливать умения чемпиона — это не единственное назначение предметов. Также они должны:

1. **Эффективно работать против оппонента на линии**. Например, если у него есть умение не лечение, игроку нужно его нейтрализовать предметом с эффектом антихил как можно раньше.

2. **Предметы должны синергировать друг с другом**. Например, все предметы связки BoRK + Guinsoo's Rageblade + Wit's End имеют on-hit эффекты. Кроме этого они срабатывают чаще, благодаря Guinsoo's Rageblade. Такая связка даёт сильный power spike на каждом предмете.

3. **Предметы должны синергировать с рунами**. Например, связка на on-hit дополняется руной Lethal Tempo. Она увеличивает скорость атаки и даёт ещё один on-hit эффект.

Чтобы достичь этих целей, игроку нужно хорошо знать своего чемпиона, его билды и предстоящий матчап.
...
**Первый пункт** — самый важный. Например, AD Carry редко не покупают предметы для танков и брузеров. Причина в том, что без защитных умений эти предметы малоэффективны. Они не дадут значимого выигрыша времени в командном сражении. С другой стороны, AD Carry значительно ослабит свой урон, если купит такой предмет.

**Второй пункт** — план на лейнинг. Рассмотрим чемпиона Ezreal. Его план на лейнинг — это poke. Чтобы исполнить этот план, ему нужна дополнительная мана и сильный урон от дальнобойного умения Q. Это дают предметы Manamune и Spear of Shojin. С другой стороны, Ezreal никогда не покупает предметы для раннего давления и all-in (например, BoRK). У чемпиона нет инструментов, чтобы исполнить такой план.
...
**Четвёртый пункт** — драфт. Например, Fiora предпочитает собираться в AD, чтобы усилить свой burst урон. Это даёт ей хороший потенциал для flanking в командном сражении. Но если в команде нет frontline, Fiora вынуждена его замещать. Она собирает предметы на выживаемость: Sterak's Gage, Spear of Shojin, Spirit Visage. С такой сборкой Fiora сможет выживать во frontline.

**В-пятых**, варды — это часть билда. Их надо менять в зависимости от роли, чемпиона и конкретной ситуации. Вот типичный выбор вардов по ролям:

1. Топлейнер — жёлтый, синий или линза.
2. Лесник — желтый, потом линза.
3. Мидлейнер — желтый, синий или линза.
4. ADC — желтый, потом синий.
5. Саппорт — желтый, потом линза.

Назначение вардов следующее:

1. **Желтый** используется для защиты от гангов и инвейдов в свой лес. Его также можно использовать агрессивно во вражеском лесу, чтобы получить информацию о леснике противников.

2. **Синий** используется для разведки на безопасном расстоянии. Например, чтобы проверить, есть ли противник на объекте.

3. **Линза** используется для агрессивных действий: роам, ганг или инвейд.
...
Красный вард — единственный, который покупается за золото. Поэтому AP Carry и AD Carry приобретают его редкое. Такая покупка откладывает их power spike и сильно ослабляет команду в очередном сражении.

[Следующая статья](https://mobalytics.gg/blog/lol-warding-guide/) даёт подробную информацию о вардинге.

### 5.3.2 Предметы для класса AD Carry
...
Начнём с предметов для класса AD Carry. Их демонстрирует таблица 5-7.
...
### 5.3.3 Предметы для класса AP Carry

Рассмотрим предметы для класса AP Carry. Как и предметы для AD Carry они делятся на три основные категории: начальные, транзитные, завершающие. Их демонстрирует таблица 5-8.
...
### 5.3.4 Предметы для класса Tank
...
### 5.3.5 Предметы для класса Assassin
...
### 5.3.6 Предметы для класса Bruiser
...
### 5.3.7 Предметы для класса Support

>>>W

### 5.3.8 Подготовка билдов
