...
Первую часть мы уже рассмотрели. Вторая относятся к билду.
...
Руны и заклинания игрок выбирает один раз. Они остаются неизменными до конца партии. В этом разделе мы рассмотрим, как правильно их выбирать.

### 5.2.1 Руны

>>>R2

Начнём с первой составляющей билда — рун. Они дают чемпиону дополнительные эффекты, усиления и предметы. Некоторые из них доступны сразу. Для получения других должно пройти время или надо выполнить какие-то условия.

Комбинация рун игрока и его оппонента определяет первые минуты игры. Именно тогда решается, кто первым получит преимущество на линии. От этого часто зависит исход всей стадии лейнинга. Таким образом правильно подобранные руны могут значительно упростить игру.

Руны меняют время и силу power spike чемпиона. Соответственно, меняются и его возможности на линии, в мидгейме и лейтгейме. Если правильно выбрать руны, можно адаптировать чемпиона к неудобному матчапу или слабой композиции команды.

Структура рун устроена следующим образом. Все они разделены на пять веток. В каждой ветке есть от трёх до четырёх основных или ключевых рун. Они называются **keystone** (краеугольный камень). Игроки называют их **тир 1 рунами**.

После keystone идёт девять дополнительных рун. Они разделены на три группы, которые называются слотами (**slot**). Игроки называют их **тир 2** (slot 1), **тир 3** (slot 2) и **тир 4** (slot 3) рунами.
...
Когда игрок выбирает руны, он указывает две ветки: основную и дополнительную. В основной ветке он выбирает одну ключевую руну. Далее в каждом из трёх слотов берётся одна дополнительная руна. В сумме их будет три. После основной ветки игрок выбирает дополнительную. В ней он может выбрать только две руны из slot 1, slot 2  и slot 3. В конце игрок берёт по одному осколку в каждом из трёх слотов.
...
4. **Resolve** (решимость, зелёная) — увеличивают выживаемость, дают механизмы для перекладки здоровья чемпиона в урон, усиливают умения на контроль и увеличивают урон по башням.
...
I> В начале сезона разработчики меняют ветки рун. Они удаляют одни руны и полностью перерабатывают другие. При этом идея каждой ветки остаётся неизменной. Уточняйте их текущее состояние на [Wiki странице](https://wiki.leagueoflegends.com/en-us/Rune).
