...
I> **Mark** (отмечать) — это приём против flanking. Его идея в том, чтобы давать обзор на противника в засаде и вытеснять (zoning) его из сражения. Тогда он не сможет атаковать союзный backline.
...
5. **Мобильность** (mobility) — выигрывает время для команды, если комбинируется с контролем. Без этого мобильность выигрывает время только для владельца умения. Про инициацию в сражении мы поговорим далее.
...
![Классификация чемпионов](images/Theory/champion-classes.png)
...
Первая стадия сражения — это **подготовка**. Цель команды на этой стадии — получить преимущество любого типа до начала предстоящего сражения. Эта цель достигается выполнением следующих шагов:
...
**Позиционирование** — это второй шаг подготовки. Прежде всего к месту сражения должны подойти все чемпионы, которые находятся в зоне доступности. Они занимают выгодную позицию, если оказались на объекте первыми. Это может быть позиция около узкого прохода или в кустах. В первом случае противник лишается пространства для маневра. Во втором — ему сложнее атаковать backline чемпионов, которые скрываются в кустах. Выгодное позиционирование даёт существенное преимущество во время сражения.
...
Вторая стадия сражения — это **инициация**. Цели команды на этой стадии — вывести из боя одного или нескольких противников и выиграть размен по урону. Чтобы достичь эти цели, команда выполняет следующие шаги:
...
Помимо умений типа hook есть другие инструменты, чтобы нарушить построение противника. Например, Jarvan IV R создаёт стены и раздвигает противников по разные стороны от них. Другой пример — Fiddlesticks Q-R комбо. Все противники в радиусе R получаю эффект страха и двигаются от Fiddlesticks. Когда построение нарушено, frontline чемпионы больше не могут защищать свой backline. Это даёт атакующим большое преимущество.
...
Третья стадия сражения — это **столкновение**. Цели команды на этой стадии — реализовать свой урон и выиграть размен по убийствам. Чтобы достичь эти цели, команда выполняет следующие шаги:
...
Заключительная стадия сражения — **преследование**. Цель команды на этой стадии — сделать как можно большей убийств. Эта цель достигается выполнением следующих шагов:
...
Если команда выбирает преследование, сначала она решает задачу **разведки**. Отступающие покидают место сражения, поэтому они оказываются вне видимости вардов. Преследователи должны проводить разведку на расстоянии. Средства для этого мы рассмотрели в разделе "2.3.5.1 Подготовка".
...
Мы рассмотрели все возможные стадии сражения. На каждой из них команда будет выполнять все шаги только тогда, когда ей выгодно играть медленно. В противном случае, события будут развиваться стремительно. Тогда команда пропускает некоторые невыгодные для неё шаги или стадии.
...
Действие любого чемпиона будет наиболее эффективным только в определённой стадии сражения. Эта стадия зависит от роли чемпиона. Каждая роль диктует определённый стандартный план действий. Он даёт максимальную выгоду в большинстве случаев. Поэтому игроку следует исполнять этот план почти всегда. Об исключениях, когда от него стоит отклониться, мы поговорим далее.

Рассмотрим несколько стандартных планов действий:
...
Начнём с первого пункта — **композиция своей команды**. Она определяет план команды на сражение. Игрок должен его понимать. Тогда он сможет подстроиться под скорость игры, которая выгодна большинству союзников. Тем самым действия команды в целом станут эффективнее.
...
**Принятие решений** — один из пяти навыков игрока, как эксперта. Мы говорили о нём в разделе "1.1.3 Навыки игрока".
