...
Большинство командных сражений в игре проходят в быстром темпе. Поэтому начинающим игрокам кажется, что события развиваются хаотично, в них нет какой-либо структуры и всё решает удача. На самом деле это не так. Мы рассмотрели все стадии сражения — это и есть его структура. Игрок должен научиться различать эти стадии. Тогда ему станет понятно, что именно происходит в каждый момент времени.

Рассмотрим навыки игрока, от которых зависит его эффективность в сражении.

#### 2.3.7.1 Знать свою роль
...
Роль чемпиона в сражении определяет, на какой стадии его атака будет самой эффективной. Это план действий игрока по умолчанию. Его следует исполнять всегда кроме случаев, когда в команде есть ключевой чемпион. Тогда надо играть вокруг него.

Рассмотрим несколько примеров для действий по умолчанию:
...
* **Чемпион на engage** должен инициировать сражение. Если он не сделал это в соответствующую стадию и началось столкновение, его врыв больше не даст команде значительного преимущества.

Если игрок не делает ход на правильной стадии сражения, он упускает возможность реализовать своего чемпиона. Запоздалое действие не будет таким эффективным, каким могло быть. Упущенная возможность часто означает, что противник получит инициативу и преимущество. Это важно учитывать. Поэтому в некоторых случаях после упущенного хода команде лучше сразу отступить.

#### 2.3.7.2 Знать композиции команд

Второй важный навык — **понимать композицию своей и вражеской команды**. Это понимание складывается из нескольких вещей.

**Во-первых**, игрок должен знать к какому из трёх типов композиции относятся чемпионы союзников. Он должен подстраиваться под темп и план на сражение, который выгоден большинству в команде. Рассмотрим пример.

Допустим, что чемпион игрока рассчитан на engage. При это все остальные чемпионы в команде ориентированы на poke. Игроку выгоден быстрый темп сражения, а всем его союзникам — медленный. В этом случае игрок должен подстраиваться. Он не должен врываться в первого попавшегося противника. Ему нужно дождаться пока союзники реализуют свой урон на дистанции. Если игрок атакует слишком рано, он погибнет и его команда останется в меньшинстве.

Общее правило звучит так:

> Если backline команды сильнее чем у противников, надо играть вокруг него.

Это означает, что игрок отказывается от своего плана на сражение, строится во frontline и выигрывает время для backline.

**Во-вторых**, игрок должен знать ключевых чемпионов в своей команде. Это те, кто создаёт решающее преимущество. Вот несколько примеров:
...
Все эти чемпионы решают исход сражения. Поэтому игрок должен подстраиваться под их план. Другими словами, он должен делать то, что выгодно ключевому союзнику.

Рассмотрим пример. Допустим, что чемпион игрока — это Aatrox. По умолчанию его план на сражение — flanking. Но в команде есть очень сильный Aurelion Sol, который достиг power spike. В этом случае игроку стоит отказаться от своего плана. Намного выгоднее будет построиться во frontline и защищать AP Carry. Это даст Aurelion Sol время, чтобы нанести максимальный урон в сражении.

Другая ситуация: в команде есть очень сильный чемпиона на engage (например, Vi). В этом случае игроку на Aatrox выгоднее отказаться от flanking и отыгрывать роль follow up. Врыв за сильным союзником и игра вокруг него будет намного эффективнее, чем самостоятельная атака вражеского backline из засады.

**В-третьих**, игроку следует знать ключевых чемпионов вражеской команды. Это те кто создают решающее преимущество, но на стороне противника. Игрок должен мешать им реализовать свои инструменты. Этого можно добиться несколькими способами:
...
#### 2.3.7.3 Работа с умениями

>>>

#### 2.3.7.4 Шаблоны типичных ситуаций
