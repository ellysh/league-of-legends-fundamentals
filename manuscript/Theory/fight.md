...
Мы рассмотрели возможные стадии сражения. На каждой из них команда будет выполнять все действия только тогда, когда ей выгодно играть медленно. В противном случае, события будут развиваться стремительно. Тогда некоторые невыгодные для команды стадии просто пропускаются. Разберёмся, что означает медленная и быстрая игра в контексте сражения.

Концепция темпа актуальна не только для партии в целом, но и для сражения любого масштаба. **Темп** — это скорость развития событий в сражении. **Играть медленно** — означает не торопить события и выжидать удачный момент для следующего хода. В медленной игре преимущество получает команда, которая успешнее реализует свои инструменты.
...
**Играть быстро** в сражении означает сразу переходить к той стадии, которая выгодна для команды. Обычно это стадия инициации или столкновения. В быстрой игре преимущество получает команда, которая первой применяет свои атакующие инструменты.
...
Большинство командных сражений в игре проходят в быстром темпе. Поэтому начинающим игрокам кажется, что события развиваются хаотично, в них нет какой-либо структуры и всё решает удача. На самом деле это не так.

Мы рассмотрели все стадии сражения — это и есть его структура. Игрок должен научиться различать эти стадии. Тогда ему станет понятно, что именно происходит в каждый момент времени и что ему следует делать.
...
Первое и самое важное — игрок должен **знать роль своего чемпиона в сражении**. Эта роль складывается из следующих параметров чемпиона:
...
Таблица 2-6 приводит эти параметры для каждого класса.

От роли чемпиона в сражении зависит, на какой стадии его атака будет самой эффективной. Это определяет план действий игрока по умолчанию. Его следует исполнять почти всегда. Об исключениях мы поговорим далее.

Рассмотрим несколько примеров действий по умолчанию:

* **Чемпион на poke** не должен терять времени на стадии подготовки. Его задача — постоянно атаковать противника с безопасной дистанции. Если чемпион этого не делает, он не реализует свой урон.

* **Чемпиона на flanking** должен заранее занять позицию в засаде. Ему незачем участвовать в построении frontline. Там он не сможет эффективно реализовать свои инструменты и его вклад в сражение будет посредственным.

* **Чемпион на engage** обычно должен начинать сражение. Если он не сделал это в соответствующую стадию и началось столкновение, его врыв больше не даст команде значительного преимущества.

Если игрок не делает ход в подходящую стадию сражения, он упускает возможность реализовать своего чемпиона. **Запоздалое действие** не будет таким эффективным, каким могло быть. Упущенная возможность часто означает, что противник получит инициативу и преимущество. Поэтому в некоторых случаях после упущенного хода команде лучше сразу отступить.

Другая ошибка, которую часто допускают игроки — это **преждевременное действие**. Она заключается в том, что игрок использует ключевые умения чемпиона сразу после начала командного сражения. В результате умение применяется неэффективно.

Концепция **holding spells** (придерживать умения) говорит о том, что медленное разыгрывание сражений часто выгоднее. Когда игрок не торопится с применением своих инструментов, с течением времени он получает больше информации. Например:

1. Кто из противников присоединится к сражению?

2. Кто из союзников присоединиться к сражению?

3. Какие ресурсы есть у противников?

До тех пор пока игрок не применил ключевое умение, он создаёт угрозу для противников. На такое давление они вынуждены реагировать. Типичная реакция — выбор безопасной позиции подальше от чемпиона игрока. Это мешает противникам реализовать свои инструменты и выгодно для команды.
...
Идеальный момент для применения ключевых умений наступает, когда противник ошибается. Вот несколько примеров таких ошибок:

1. Несколько противников оказались рядом друг к другу. В этом случае сильное умение с уроном по области (AoE) может перевернуть ход сражения.
...
4. Бэйт противников на невыгодное применение ключевых умений. Например, engage во frontline чемпиона с хорошей выживаемостью.
...
Ошибки противника открывают окно возможностей, когда игрок может реализовать свои инструменты наиболее эффективно. Поэтому ему выгоднее ждать с применением умений. Это можно делать до тех пор, пока идёт стадия сражения, на которой он должен действовать. Если эта стадия уже подходит к концу, игроку лучше применить свои инструменты.

#### 2.3.7.2 Знание композиций команд

Второй важный навык — **понимать композицию своей и вражеской команды**. Для этого игрок должен уметь определять следующее:
...
**Во-первых**, композиция команды определяет её план на сражение. Игрок должен её понимать. Тогда он сможет подстроиться под темп и план, который выгоден большинству чемпионов в команде. Рассмотрим пример.

Допустим, что чемпион игрока рассчитан на engage. При это все остальные чемпионы в команде ориентированы на poke. Игроку выгоден быстрый темп сражения, а его союзникам — медленный. В этом случае игрок должен подстраиваться. Он не должен врываться в первого попавшегося противника. Ему нужно дождаться пока союзники реализуют свой урон на дистанции. Если игрок атакует слишком рано, он погибнет и его команда останется в меньшинстве.

**Во-вторых**, игрок должен знать ключевых чемпионов в своей команде. Эти чемпионы создают решающее преимущество. Вот несколько примеров:

1. Гиперкерри чемпион, который достиг свой power spike.
...
Все они могут решить исход сражения. Поэтому игрок должен подстраиваться под их план. Другими словами, он должен делать то, что выгодно ключевому союзнику.

Рассмотрим пример. Допустим, что чемпион игрока — это Aatrox. По умолчанию его план на сражение — flanking. Но в команде есть очень сильный Aurelion Sol, который достиг power spike. В этом случае игроку стоит отказаться от своего плана. Для него выгоднее будет построиться во frontline и защищать ключевого союзника. Это даст Aurelion Sol время, чтобы нанести максимальный урон в сражении.
...
**В-третьих**, игроку следует знать ключевых чемпионов вражеской команды. Они создают решающее преимущество на стороне противника. Игрок должен мешать им реализовать свои инструменты. Этого можно добиться несколькими способами:

1. **Угрожать разменом**. Игрок может потратить все свои ресурсы (flash, ультимативное умение и т.д.) на внезапную атаку ключевого противника.
...
Когда идёт сражение, важно принимать решения быстро. Запоздалое или преждевременное действие может привести к поражению. Когда наступает стадия столкновения, каждая секунда времени становится очень дорогой. Чемпионы frontline разменивают свои динамические ресурсы на время. Пустая трата этих ресурсов на раздумье или бездействие даёт противнику преимущество.

Навык принятие решений напрямую связан с умением группировать информацию и обнаружением в ней шаблонов. Игроки учатся этому из опыта. Получить этот опыт можно несколькими способами:

1. Метод проб и ошибок
2. Коучинг
3. Самокоучинг.

**Метод проб и ошибок** — самый распространённый подход. Он заключается в том, что игрок пробует разные действия в сражении на своём основном чемпионе. Когда действие сработало хорошо, игрок запоминает текущие условия партии в качестве шаблона. Он повторит это действие в следующий раз, когда встретятся похожие условия. При этом игрок не будет долго раздумывать. Ситуация окажется ему знакома и он быстро отреагирует на неё.

**Коучинг** позволяет выучить шаблоны типичных игровых ситуаций быстрее. Коуч указывает игроку на ошибки и говорит, какое действие в текущей ситуации будет правильным. Таким образом игрок получает уже готовый шаблон и правильную реакцию на него. Его задача — запомнить информацию и отработать её в нескольких партиях. Тогда в похожих ситуациях игрок будет принимать решения быстро.

**Самокоучинг** — нечто среднее между первыми двумя способами. При просмотре реплея у игрока есть время, чтобы подумать над конкретной ситуацией в партии. Он может оценить все важные факторы, которые влияют на исход сражения: роль своего чемпиона, композиции обеих команд, их ключевые чемпионы и умения. Из этого игрок может сделать вывод о своём правильном действии. Если он попробует это действие в следующей партии и оно не даст положительного результата, сессию самокоучинга следует повторить.

Когда идёт сражение у игрока нет времени, чтобы раздумывать. Если он знает шаблон текущий ситуации, он быстро выполнит правильное действие. Если шаблон незнаком, игрок вынужден действовать интуитивно. Часто это приводит к ошибкам. Коучинг и самокоучинг позволяют их корректировать и заучивать такие ситуации в качестве шаблонов.
...
Оно означает, что игрок должен фокусироваться не только на убийстве противников. Давление на них не менее важно, чем такие очевидные параметры, как нанесённый и полученный чемпионом урон. Другими словами, размен игрока на одного противника редко оказывается лучшим выбором. Лучше оставаться живым как можно дольше и иметь возможность повлиять на события.
...
Как эффективно управлять вниманием и бороться с эффектом туннельного зрения? **Во-первых**, игрок должен концентрироваться на своей роли в командном сражении. В первую очередь он должен обращать внимание на цели, с которыми связан его план. Рассмотрим пример.

Допустим, что чемпион игрока — это Disengage Tank для frontline. Его первоочередная задача заключается в том, чтобы защищать свой backline. Поэтому игрок должен обращать больше внимания не на вражеских чемпионов, а на своих керри. Если для них возникнет угроза с любого направления, её надо немедленно нейтрализовать.
