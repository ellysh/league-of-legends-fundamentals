...
Основы всегда кажутся тривиальными. Тем не менее многие начинающие игроки плохо их понимают. Это мешает им освоить конкретные практические приёмы (например, wave management). Пока игрок не поймёт общие закономерности в механике League of Legends, информация из гайдов будет казаться ему бессвязной и хаотичной. Усвоить её в такой форме практически невозможно.
...
Мы рассмотрели три формы равновесия игровых объектов. Каждое из них является **динамическим**. Это означает, что если его нарушить, оно вернётся в исходное состояние со временем. Рассмотрим примеры.
...
**В-третьих**, фарм вражеских миньонов — это стандартное действие на стадии лейнинга. Оно сохраняет равновесие по ресурсам между игроками на линии. Если синий мидлейнер лучше добивает миньонов, он получит преимущество с минимальным риском. В данной ситуации это будет наилучшим действием.
...
4. **По ресурсам** — у одного или нескольких чемпионов команды есть что-то из следующего, когда у оппонентов этого нет:
   ...
   * индивидуальные или командные [**усиления**](https://leagueoflegends.fandom.com/ru/wiki/Усиление), полученные за убийство лесных монстров.
...
**Реализация преимущества** — это самое выгодное действие в общем случае. Команда или отдельный чемпион уже сильнее противника. Они должны как-то это реализовать и получить что-то в награду. Пример: атаковать противников на объекте, когда у команды есть численное преимущество.
...
I> В английском языке есть разные термины для когнитивных шаблонов (template) и закономерностей в воспринимаемой информации (pattern). Оба термина переводятся на русский как "шаблон". Далее в книге мы будем использовать слово шаблон для обоих понятий.

В **стратегиях реального времени** (real-time strategy или RTS) есть понятие макроцикла. **Макроцикл** (macro cycle) — это подход к эффективному управлению игровой экономикой. Его идея в том, чтобы составить список задач и последовательно его исполнять.
...
Рассмотрим макроцикл мидлейнера синей команды. На иллюстрации его чемпион отмечен розовым кругом с номером 1. Розовый текст указывает объекты, проверка которых входит в макроцикл игрока.
...
2. Состояние оппонента на линии:
   ...
   * готовые руны (надо запоминать время перезарядки)
   * предметы.
...
Лесник синей команды отмечен розовым кругом с номером 1. Согласно иллюстрации, его макроцикл выглядит так:
...
Начните тренировать макроцикл в режиме "Co-op vs. AI" (против ботов). Так вам будет проще следить за переключением внимания. Скоро минимальный макроцикл войдёт у вас в привычку. Добавьте в него действия, которые считаете важными. Постепенно сокращайте задержку между звуковыми сигналами.

Распределять внимание между важными аспектами игры необходимо для каждой роли. Особенно это справедливо для лесника. Если лес — ваша основная роль, постарайтесь освоить минимальный макроцикл перед следующей рейтинговой игрой.
...
League of Legends — это игра в реальном времени. Её механика позволяет игрокам действовать непрерывно. Единственное исключение — это смерть. В этом случае игрок должен подождать таймер своего воскрешения. Тем не менее, в игре есть понятия хода. Он называется плэй.

I> **Плэй** (play) — отдельное действие команды или игрока, которое преследует одну конкретную цель. Например, взятие эпического монстра или разрушение вражеской башни.
...
Для простоты coach Kairos предлагает учитывать только часть статических и динамических ресурсов чемпиона. Их приводит таблица 2-1.
...
Мы выяснили, что есть разные типы преимущества. Также преимущество одного типа может отличаться количественно. Например, сравним разницу по золоту между командами в 1000 и в 14000. Во втором случае каждый чемпион лидирующей команды опережает своего оппонента на линии на целый предмет. Это открывает гораздо больше возможностей, чем небольшое преимущество команды в 1000 золота.
...
**Комбинация чемпионов противников** определяет, какие средства есть у вражеской команды. Некоторые средства могут полностью нейтрализовать возможности команды игрока. Это надо учитывать при составлении плана.

**План игрока на лейнинг** определяет, как именно он собирается получить преимущество на стадии лейнинга. Некоторые чемпионы могут смещаться и помогать союзникам в течение лейнинга. Другие вынужденны постоянно оставаться на своей линии. В противном случае, они выйдут в мидгейм с отставанием. Это тоже надо учитывать в общем плане на игру.
