...
Подсчитаем, какие ресурсы получают чемпионы на стадии лейнинга.
...
Обратите внимание, что 30-я волна миньонов появляется на 16-й минуте. Примерно к этому времени заканчивается стадия лейнинга.
...
Допустим, лесник провёл неудачный ганк. В результате вражеский топлейнер выжил и даже не потерял здоровье. Игроку кажется, что в этом нет ничего плохого и равновесие между командами не нарушилось. На самом деле, это не так: противники получили преимущество. Причина в том, что игрок впустую потратил своё время и недополучил ресурсы за фарм леса. Всё это время вражеский лесник фармил и поэтому он стал сильнее игрока.
...
Мы выяснили, что лесник потратил на ганк около 40 секунд. Если бы вместо этого он вернулся в лес и начал фармить, он бы успел забрать один лагерь монстров. В пересчёте на ресурсы получим примерно 100 золота и 200 опыта. Это плата лесника за участие в ганке.
...
Получился неплохой результат. Но он вряд ли решит исход стадии лейнинга.

В **худшем случае** выигрыш получит красная команда. Он сложится из следующего:

1. Красный мидлейнер получит награду за двойное убийство. Это 300 золота и 300 опыта за каждого синего чемпиона.

2. Красный мидлейнер может остаться на линии и фармить. Тогда он получит 300 золота и 600 опыта. синий мидлейнер эти ресурсы не получит.
...
Во втором примере лесник синей команды забирает [огненного дракона](https://leagueoflegends.fandom.com/ru/wiki/Огненный_дракон). Сначала он делает полную чистку своего леса во второй раз. После этого выходит на эпического монстра. Так игрок использует своё время максимально эффективно.
...
Посчитаем среднее усиление для всех AD и AP чемпионов в команде:
{line-numbers: false, format: text}
```
5 * (105 + 24) / 2 = 322.5
```
...
Соотнесём награду лесника за дракона с его обычным фармом монстров в течение минуты: 300 золота и 600 опыта. Получается, что за взятие дракона лично он заплатил `300 - 125 - 105 = 70` золота и `600 - 150 - 80 = 370` опыта. Его команда получила в сумме `320 - 70 = 250` золота.
...
Рассмотрим худший сценарий взятия дракона. Допустим, что синий мидлейнер и ADC помогают своему леснику взять объект. В этом случае они забирают дракона примерно за 20 секунд. Ещё 10 секунд они потратят, чтобы вернуться на свои линии. Их суммарная плата по золоту за участие в командном действии составит:
{line-numbers: false, format: text}
```
150 + 150 + 150 = 450
```
...
На начальной стадии игры это серьёзное вложение. Как мы посчитали, команда получит за дракона 425 золота и 230 опыта. То есть награда почти скомпенсирует плату золотом за участие в командном действии, но не опыт.

Теперь допустим, что лесник красной команды ждёт в засаде, пока синие чемпионы бьют дракона. Когда у монстра остаётся мало здоровья, красный лесник его добивает [заклинанием кара](https://leagueoflegends.fandom.com/ru/wiki/Кара_(Заклинание)) (smite) и выживает после этого. В этом случае синие чемпионы не компенсируют свою плату за участие в командном действии.
...
Перевес красной команды по золоту получится такой:
{line-numbers: false, format: text}
```
450 + 425 = 875
```
...
Это означает, что кража дракона даёт противнику большое преимущество. В начале партии скомпенсировать его будет очень трудно.
...
Вернёмся к нашему примеру с разрушением башни. Допустим, что синяя команда забирает барона Нашора. Красная команда пытается этому помешать. В результате происходит сражение. В нём погибают три красных чемпиона и два синих. Тогда красные отступают, а синие добивают барона Нашора.
...
Максимальная награда по золоту за двух красных чемпионов составит:
{line-numbers: false, format: text}
```
450 + 450 = 900
```
...
Для качественного самокоучинга новичкам не хватает знаний. Расчёт ресурсов — это универсальный метод для оценки качества решений. Для начинающих игроков он может стать хорошей отправной точкой, чтобы начать самокоучинг. Рассмотрим на примере, как это работает.
