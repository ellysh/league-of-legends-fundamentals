...
2. **Предметы должны синергировать (сочетаться) друг с другом**. Дальше мы рассмотрим конкретные примеры.

3. **Предметы должны синергировать с выбранными рунами**. Такие примеры тоже будут дальше.

Чтобы добиться всех трёх целей, необходимо хорошо знать матчап, а также подходящие для своего чемпиона предметы и руны.
