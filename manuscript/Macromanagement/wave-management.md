...
Proxy farming приводит к тем же результатам, что и приём push. Но они зависят от решений противника.
...
2. **Если оппонент допускает креш, появляется возможность для дайва**. Proxy farming отрезает противнику пути отступления, когда несколько союзников готовят дайв.

3. **Позволяет фармить лагеря лесных монстров на стороне карты противника**. Это создаёт перевес по ресурсам для команды игрока.
...
1. **Противник получает преимущество по динамическим ресурсам, если допускает креш**. Игрок же вынужден танковать миньонов и теряет здоровье. В случае дуэли он будет слабее.

2. **Противники могут атаковать игрока с численным преимуществом**. Для этого могут сместиться мидлейнер и лесник.
