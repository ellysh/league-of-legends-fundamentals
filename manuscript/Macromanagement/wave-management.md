...
Рассмотрим общие правила для выбора тайминга recall:

1. Recall выгоднее делать, когда с обеих сторон подходит волна с осадным миньоном. Такие волны умирают медленнее. Это означает, что у игрока есть больше времени на перемещение по карте.

2. Когда большая союзная волна пушится в сторону противника, игрок должен находиться на линии. В этом случае вражеских миньонов меньше и они умирают быстрее. Поэтому у игрока нет времени перемещаться по карте.

3. Когда большая вражеская волна пушится в сторону игрока, он может сделать recall. В этом случае вражеских миньонов меньше, они умирают медленнее и накапливаются. Игрок может прийти позже и забрать сразу полторы волны.

4. Recall должен решать несколько задач. Желательно сразу трёх: восстановление здоровья, маны и покупка предмета. Если recall решает только одну задачу, на стадии лейнинга он невыгоден.

Обратите внимание, что эти правила справедливы, когда противника нет на линии. В противном случае он сделает recall игрока невыгодным. Например, выполнит fast push и креш.

Теперь рассмотрим возможности для recall в зависимости от приёма, который исполнил игрок. При этом оппонент находится на линии. Таблица 4-3 приводит эту информацию.
...
Если recall возможен, игрок теряет минимум ресурсов. Если — невозможен, потери игрока будут значительными.

Во время исполнения slow push игрок не может покинуть линию, потому что оппонент сделает freeze. Slow push должен закончиться fast push'ем. Только тогда игрок сможет сделать recall.
