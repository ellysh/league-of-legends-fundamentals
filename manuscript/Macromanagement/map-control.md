...
**Первый сценарий** — игрок выполняет push и забирает башни на боковых линиях. Противники игнорируют угрозы. Они контестят объекты с численным преимуществом в других частях карты. В результате команда игрока несколько раз погибает в сражениях.

Такой сплит-пуш будет невыгоден. Противники получают объекты и награды за убийства. С другой стороны, игрок — это единственный, кто получает ресурсы в своей команде. Но он никак их не реализует, потому что не участвует в командных действиях.

**Второй сценарий** — игрок выполняет push на боковой линии. Противники реагируют на угрозу и атакуют игрока втроём или вчетвером. В это время союзники получают темп на каком-то объекте, но никак его не реализуют. Например, лесник продолжает фармить лес, а кто-то делает recall.

Такой сплит-пуш тоже невыгоден. Противники просто получают награду за убийство игрока, а затем контестят очередной объект с численным преимуществом.

Из этих примеров можно сделать такой вывод:

> Игрок не может действовать по одному и тому же плану на макро уровне в каждой партии Он должен адаптировать свои действия к конкретным противникам и союзникам.

В некоторых играх сплит-пуш боковой линии окажется выгоден и даст команде преимущество на объекте. В других будет развиваться по одному из описанных сценариев.

Есть более безопасные способы создать угрозу, чем push на боковой линии. Один из них называется **ротация треугольник**.

Ротация треугольник выполняется, когда разрушена одна или обе Т1 башни на линии. Она состоит из трёх шагов:
...
Ротацию выполняет синий топлейнер. Он разрушил Т1 башню противника. при этом его Т1 башня ещё стоит.
...
**Второе действие** — ротация у месту очередного командного действия. Например, это может быть контест барона или дракона, ганг на среднюю линию, инвейд вражеского леса или телепорт на нижнюю линию для дайва. Если на карте нет командного действия, игрок может сделать recall.
...
Чтобы отреагировать на ротацию треугольник, противник должен сместиться к своей Т2 башне, забрать волну и отпушить линию фронта на сторону карты игрока. Обычно опытные игроки на лини выполняют ротацию треугольник по очереди.

#### 4.3.2.3 Приём pin и fog
...
При равенстве сил на боковой линии опытные игроки применяют приём **pin и fog**. Дословно её название переводится как "прикрепить и затуманить". Идея приёма заключается в том, чтобы в одиночку создать сразу две угрозы: на боковой линии и в ближайшем от неё вражеском лесу. Суть угрозы в том, что противник может потерять ресурсы: миньонов под своей башней и лагеря лесных монстров.

Чтобы исполнить приём pin и fog на боковой линии, на ней прежде всего надо разрушить вражескую Т1 башню. Это откроет игроку доступ во вражеский лес.

I> Профессиональный игрок [Ryan Short](https://liquipedia.net/leagueoflegends/Chippys) (известный как Chippys) детально объясняет приём pin и fog в [следующем видео](https://youtu.be/4PIDc9ZeBIw?si=UMGr3KPtgyQ1ks_O&t=310). 

Приём состоит из двух шагов. **Первый шаг** — **pin** (прикрепить). Как следует из названия, он привязывает оппонента к его башне. Чтобы выполни pin, надо сделать push и крешить союзных миньонов во вражескую Т2 башню. Это первая угроза, на которую должен отреагировать противник. Если он её игнорирует, то теряет фарм под своей башней. Если он выбирает фармить миньонов, он оказывается привязан к башне и не может отойти от неё какое-то время.
...
Распишем подробно действия игрока при выполнении приёма pin и fog:
...
Иллюстрация 4-15 демонстрирует действия игрока при исполнении приёма pin и fog на карте Summoner's Rift.

{caption: "Иллюстрация 4-15. Действия для исполнения приёма pin и fog", width: "100%"}
![Приём pin и fog](images/Macromanagement/pin-and-fog-steps.png)

Иллюстрация показывает исполнение приёма pin и fog игроком синей команды на верхней линии. Красный крест на Т1 башне красной команды означает, что она уже разрушена.
...
Допустим, противники отреагировали на обе угрозы, которые игрок создал приёмом pin и fog на верхней линии. В этом случае его команда получает темп. Его можно реализовать одним из стандартных командных действий, которые выполняются параллельно с угрозой. Мы рассмотрели их ранее в разделе "4.3.2 Угрозы противникам".
...
Иллюстрация 4-16 демонстрирует установку вардов при исполнении приёма pin и fog в лесу синего стража красной команды.
...
Запомните, что нельзя начинать какое-либо действие на стороне карты противника, если у вас нет информации о местоположении большинства вражеских чемпионов. Примеры таких действий: proxy farming, угроза push, приём pin и fog, расстановка вардов во вражеском лесу, засада во вражеском лесу. Если противников нет на карте и вы не знаете, где они, играйте медленно. Сделайте freeze волны на своей линии и ждите, пока противники покажутся на карте.
