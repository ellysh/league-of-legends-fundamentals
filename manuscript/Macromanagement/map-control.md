...
В предыдущих главах мы упоминали создание угрозы противнику. Этот приём можно определить так:
...
2. **Проигнорировать**, но потерять ресурсы или отдать их оппоненту.

Рассмотрим создание угрозы на примере приёма fast push. Если выполнить его правильно, миньоны крешатся во вражескую башню. В определённых игровых ситуациях это создаёт угрозу. Она ставит противника перед следующим выбором:
...
2. **Проигнорировать** fast push и принять участие в командном действии. Это приведёт к потере ресурсов. Кроме этого башня получит урон.

Станет ли fast push угрозой, зависит от происходящего на карте. Возможны два случая.

1. Игрок выполняет fast push, когда на карте нет никакого командного действия или подготовки к нему. Тогда противник отреагирует на угрозу и при этом ничего не потеряет.

2. Игрок выполняет fast push перед командным действием (например, контест эпического монстра). Тогда противнику сложнее сделать выбор. Отреагировать на угрозу, означает оставить команду в меньшинстве на объекте. Проигнорировать угрозу, означает дать игроку преимущество по ресурсам.
...
**Второй вывод** — реакция противников на угрозы зависит от разных факторов. Если оппонент имеет низкий ранг, он может просто не понять смысл угрозы и варианты ответа на неё. Это даёт игроку шанс извлечь из угрозы максимальную выгоду.
...
2. Команда игрока должна потратить меньше ресурсов на создание угрозы, чем вражеская команда на то, чтобы отреагировать на неё. Например, если два чемпиона синей команды пушат линию, а один красный чемпион его останавливает — это неэффективная угроза.

Только если оба условия выполняются, угроза будет эффективной.

Угрозы можно комбинировать. Если у команды высокий темп, она может создать несколько угроз в разных областях карты. Отреагировать на них сложнее. Противники должны подсчитать потери от игнорирования каждой угрозы. После этого следует реагировать на ту, которая приводит к наибольшим потерям. Начинающие игроки часто ошибаются в такой оценке и делают неправильный выбор.
