...
I> Профессиональный игрок Jake Sharwood объясняет, как исполнять перенос давления в [следующем видео](https://www.youtube.com/watch?v=TiCeoxQYCuk).
...
I> Профессиональный игрок Jake Sharwood объясняет, как исполнять push по двум линиям в [следующем видео](https://youtu.be/TiCeoxQYCuk?si=oVK24o9bchOCumV8&t=425).
...
Контест эпических монстров — важная часть игры в мидгейме и лейтгейме. Результат такого контеста часто определяет исход партии. Рассмотрим, правильный подход к решению этой задачи.

#### 4.3.4.1 Подготовка к объекту

>>>R1

Подготовка к контесту эпического монстра начинается с правильных таймингов для recall. Возвращаться на базу следует за 30-60 секунду до возрождения монстра. Конкретный тайминг зависит от чемпиона игрока.

I> Профессиональный игрок Ryan Short рассказывает про подготовку к объекту в [следующем видео](https://youtu.be/4PIDc9ZeBIw?si=qK6VjCyXpi0eEdB6&t=745).

Если чемпион играет back-to-front в сражении, он должен прийти на объект заранее. Его тайминг для recall — это 60-70 секунд до возрождения монстра. Тогда у игрока будет достаточно времени, чтобы дойти до объекта и занять выгодную позицию для flanking.

Если чемпион играет front-to-back в сражении, он может прийти на объект немного позже. Его тайминг для recall — это 30-40 секунд до возрождения монстра.
...
2. **Расставить варды** в ключевых точках.

Push линий сдвигает фронт столкновения миньонов на сторону карты противника. Таким образом миньоны дают обзор на перемещения противников. Для подготовки к контесту дракона надо отпушить среднюю и нижнюю линии. Для контеста герольда или барона пушится верхняя и средняя линии.

Рассмотрим ключевые точки для установки вардов на объекте. Начнём с дракона. Их демонстрирует иллюстрация 4-22.
...
Самые важные точки имеют номера 1 и 2. Первая для розового варда находится внутри логова дракона. Если установить его правильно, он выключит варды противника в логове. Вторая точка для жёлтого или розового варда находится в ближайших к логову кустах. Их часто используют для засады. Поэтому контроль над ними может решить исход командного сражения.
...
Идея установки вардов такая же, как в логове дракона. Самые важные точки с номерами 1 и 2. Первая — розовый вард внутри логова. Вторая — жёлтый или розовый вард в ближайших кустах, чтобы предотвратить засаду.

Точка номер 3 важна для синей команды. Желтый вард там предотвращает кражу барона.

Точки 4 и 5 перекрывают альтернативный путь к логову монстра со стороны вражеской базы. Красная команда ставит жёлтый или синей вард в точке 5, синяя команда — в точке 4.

I> [Следующее видео](https://www.youtube.com/watch?v=U-uju2fG15k) подробнее объясняет установку вардов на объектах.

Если команда приходит на объект заранее, у неё появляются возможности для более агрессивных действий:

1. Устроить засаду во вражеском лесу на подходе к логову эпического монстра.

2. Ганг противника, который пушит боковую линию перед объектом.

3. Атаковать противника, который смещается к объекту и отстал от команды.

На низком Эло обзор вокруг объекта также важен. Он даёт информацию, когда противники начали забирать монстра. Часто игроки делают это без темпа, за что их можно легко наказать. Лучшим решением будет устроить засаду и атаковать, когда вражеская команда получила значительный урон от монстра и оказалась на низких ресурсах.

#### 4.3.4.2 Координационные игры вокруг объекта

>>>R1

Взятие эпического монстра — достаточно опасное действие для обеих команд. Обычно объект пытается забрать сторона, которая в данный момент имеет преимущество. Если события будут развиваться неудачно, команда может отдать не только эпического монстра, но и всё своё преимущество. С другой стороны, если отстающая команда проиграет сражение на бароне, это часто заканчивается поражением в игре.

На высоком Эло усиления эпических монстров становятся условием победы. Поэтому вокруг них происходит ряд координационных игр. С их помощью опытные игроки стараются получить преимущество непосредственно перед сражением на объекте.
...
Кратко рассмотрим суть первой и второй координационной игры. Третью мы рассмотрим подробнее на примере.

**Расстановка вардов**. Мы рассмотрели основные точки для установки вардов на иллюстрациях 4-22 и 4-23. Часто у команды не хватает времени, чтобы завардить и задевардить все эти точки. Поэтому в некоторых из них может остаться вражеский вард. Если это розовый вард на подходе, он позволит противникам подойти к объекту незаметно и напасть с тыла. Если это жёлтый вард в логове, он позволит противникам украсть объект. Суть координационной игры в следующем. Команда приходит на объект первая и решает дилемму: быстрее забрать объект или потратить время на вардинг и девардинг.

**Попытка украсть эпического монстра**. Обычно происходит как размен: вражеский лесник врывается в логово монстра и добивает его заклинанием кара. После этого он погибает. Но лесник должен угадать подходящий момент, чтобы у монстра оставалось 1200 здоровья. Суть координационной игры: забирать объект как можно быстрее или остановиться и ждать врыв вражеского лесника.

Теперь рассмотрим координационную игру **засада перед объектом**. Она сложнее и её исход влияет на ход партии значительнее, чем исход других игр.

Представим следующую игровую ситуацию. Синяя команда имеет преимущество и темп на бароне. Все чемпионы красной команды находятся на карте в зоне доступности радиусом 12000 от барона. Это примерно 30 секунд пути в мидгейме.

Синяя команда выбирает между следующими действиями: пытаться забрать барона как можно быстрее, либо устроить засаду во вражеском лесу. Первый вариант опасен тем, что у красной команды может быть обзор на бароне. Тогда они узнают о действиях синей команды, подойдут на объект и нападут с тыла. Второй вариант в худшем случае приведёт к тому, что синяя команда простоит в кустах, пока противники пушат линии. Тогда они получат фарм и могут разрушить одну-две синие башни.
...
Столбцы таблицы соответствуют действию красной команды. Строки таблицы — это действия синей команды. Ячейки таблицы содержат оценки результата от выбранных действий для обеих команд. Оценки имеют следующие значения:

* 0 — плохой результат
* 1 — средний результат
* 2 — хороший результат.

Возможны четыре результата игры:
...
2. **Синяя команда атакует барона, а красная пушит линии**. Выигрыши будут 2 на 1 в пользу синих. Красные получат фарм и золото за разрушенную башню. Но усиление барона будет выгоднее.
...
4. **Синяя команда устраивает засаду, а красная пушит линии**. Выигрыши будут 0 на 2 в пользу красных. Синие теряют время, в то время как красные фармят и разрушают башни.

Из таблицы 4-11 можно сделать следующие выводы:

1. Синей команде выгодно забирать барона, когда красные его отдают. Если красные намерены контестить объект, выгоднее утроить засаду.

2. Красным выгоднее смещаться на объект, если синие его забирают. В противном случае выгоднее пушить линии.

Ставки в этой координационной игре очень высоки. Команда забравшая барона сможет закончить игру или получить решающее преимущество. Поэтому лучшим решением для обеих команд будет не рисковать и отказаться от координационной игры. Это можно сделать с помощью разведки.

Начнём с вариантов для красной команды. **Во-первых**, голубой вард в логово барона сразу покажет — начала ли синяя команда забирать объект. Если её нет в логове и на линиях, значит она устроила засаду. В этом случае будет выгоднее отпушить линии. После этого можно снова проверить логово барона.

**Во-вторых**, дальнобойные умения чемпионов позволяют проверить, есть ли противники в кустах. Такие умения могут прокнуть руну, эффект предмета или пассивное умение чемпиона. Это укажет на попадание по цели.

**В-третьих**, на высоком уровне Эло разведку проводит саппорт с активированной линзой. На 16 уровне чемпиона она даёт просветку с радиусом 575. Такая дистанция позволит игроку с хорошей реакцией уклониться от умений противников, которые скрываются в кустах.

Варианты разведки для синей команды следующие. **Во-первых**, отпушить верхнюю и среднюю линии заранее. Только после этого смещаться на объект. Тогда миньоны покажут, какое решение приняла красная команда: смещаться или пушить. Если красные пушат, синие могут попытаться быстро забрать барона. В противном случае, им лучше устроить засаду.

**Во-вторых**, расставить варды на подходе к объекту. Это могут быть точки 3 и 4 на иллюстрации 4-23 или более глубокие варды во вражеском лесу. Они покажут, решилась ли красная команда на контест.

Разведка срабатывает не в каждой координационной игре. Если это случилось, лучше следовать совету опытных игроков:

> Выигрывают те, кто меньше ошибается. Поэтому менее рискованные решения дают больше побед в долгосрочной перспективе.

На практике это означает следующее. Во-первых, постройте в уме таблицу выигрышей координационной игры, которая у вас сложилась. Во-вторых, выберите в ней наименее рискованное решение, исходя из конкретной игровой ситуации.
...
Качественная подготовка увеличивает шансы на победу в сражении. Её проще всего выполнить перед контестом объекта. В этом случае действия сторон и их перемещения более предсказуемы. Поэтому если объект не очень важный, игрокам следует концентрироваться не на его взятии, а на сражении за него.

Рассмотрим объект, как повод для командного сражения. Тогда начинать его забирать выгоднее команде с преимуществом. Например, это может быть лучший темп на объекте. Команда хочет сражаться, чтобы реализовать своё преимущество. Её смещение на объект провоцирует противников ответить тем же.
...
Оба преимущества временные. Их нет у команды, которая заходит на объект вторым номером. Но спустя 5-7 секунд она установит варды и сгруппируется для сражения. Поэтому чтобы реализовать преимущества, нужна хорошая координация и быстрое принятие решений.
...
Синяя команда заметила отсутствие на линии красного мидлейнера и ботлейн. У них них нет обзора на драконе. Игроки не знают наверняка, что происходит на объекте, но решают смещаться на него. Точка 2 — это ADC и саппорт синей команды. Они двигаются на объект по кратчайшему пути снизу. Точка 3 — это синий лесник и мидлейнер. Они заходят сверху.

В сложившейся ситуации у красной команды есть оба преимущества: обзор на объекте и выгодная позиция. Чтобы их реализовать, игроки должны сделать следующее:
...
На низком уровне Эло эти преимущество гораздо легче реализовать в сражении. Причина в том, что игроки плохо координируют свои действия и им нужно больше времени на принятие решений.
