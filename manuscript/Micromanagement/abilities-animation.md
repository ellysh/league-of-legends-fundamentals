...
Маскировать анимацию можно не только с помощью flash, как в комбо Ezreal Q-F и R-F. Для этого подходят и некоторые статические умения.
...
**Комбинация Jinx E-R** решает обе проблемы умения R. Умение E маскирует анимацию R windup. Таким образом противник не видит, что чемпион выполняет подготовку. Это скрывает момент, когда  Jinx становится уязвима в сражении из-за lockout move. Кроме этого противник не знает направление запуска ракеты.
...
В точке C начинается фаза анимации R firing.Она длится 0.07 секунды. Эта фаза так же маскируется анимацией E recovery.

В точке D заканчивается анимация R firing. С этого момента Jinx проигрывает только оставшуюся часть фазы E recovery. Вся фаза E recovery занимает примерно 1 секунду. Обратите внимание, что именно она маскирует анимацию R windup и R firing на отрезке B-D.

В точке E заканчивается анимация E recovery. После этого Jinx проигрывает оставшуюся часть анимации R recovery. На отрезке D-E её маскировала анимация E recovery. Полная длительность R recovery на отрезке D-F составляет 1.12 секунды.
...
Второй пример продемонстрирует маскировку модификатора атаки анимацией windup статического умения. Рассмотрим **комбо Darius W-Q**. Его демонстрирует [следующее видео](https://www.youtube.com/watch?v=83n-RIxQ25k&t=86s).
...
В точке C заканчивается анимация W windup. Она занимает 0.33 секунды. После неё Darius проигрывает фазу W firing и наносит противнику урон. Эту фазу тоже скрывает анимация Q windup. Противник видит только то, что Darius замахивается топором. Но при этом он получает урон от умения W чемпиона.
