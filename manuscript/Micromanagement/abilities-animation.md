...
Схема базовой атаки одинакова для всех чемпионов: в ней всегда есть три фазы. Пропорции этих фаз отличаются. Например, у Volibear windup занимает 30% времени всей анимации атаки, а у Ashe — только 21.93%. Отличается также базовая скорость атаки и её прирост за уровень. Но это несущественные детали. Алгоритм исполнения приёмов attack move и attack ability один и тот же для всех чемпионов.
