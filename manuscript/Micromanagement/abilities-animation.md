...
Рассмотрим, как построить комбо на сбросе таймера автоатаки. Для примера возьмём **комбо AA-E чемпиона Vi**. Её умение E имеет оба свойства: attack reset и uncancellable windup.
...
Свойство модификатора атаки uncancellable windup позволяет сменить позицию перед фазой firing. Этот приём используется только для отступления. Он позволяет разорвать дистанцию с противниками после дайва, инициации в сражении и атаке с фланга.

Комбо со сменой позиции для отступления будем называть **защитными**. Комбо со сменой позиции для нападения назовём **атакующим**.

Смены позиции перед фазой firing работает не только с модификаторами атаки, но и с базовой атакой. Для начала познакомимся со случаем АА, потому что он проще.
...
1. В точке A игрок нажимает правой кнопкой мыши на цель. Чемпион начинает проигрывать анимацию AA windup. Она длится 0.35 секунд и заканчивается в точке C.

2. В точке B сразу после начала анимации windup игрок нажимает flash. Чемпион разрывает дистанцию с целью. В новой позиции он проигрывает оставшуюся часть анимации AA windup. Это отрезок B-C на диаграмме.

3. В точке C начинается анимация фазы AA firing. В этот момент цель получает урон. Это сработает, даже если она оказалась вне дальности атаки Vi.
...
1. Начинают комбо, когда цель находится вне дальности атаки чемпиона. Из-за этого он будет двигаться к цели. Дальше игрок на автомате нажимает flash до начала анимации AA windup. В результате чемпион не выполнит автоатаку.

2. Отменяют анимацию AA windup после flash. Например, это можно случайно сделать командой на движение. Тогда чемпион прекращает фазу windup и не выполняет автоатаку.

Вторая проблема решается свойством модификатора атаки uncancellable windup.
