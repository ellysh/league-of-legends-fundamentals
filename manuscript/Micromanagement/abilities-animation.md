...
Если анимация двух умений проходит по одной схеме, тогда с ней можно работать одними и теми же приёмами. Поэтому классификация важна. Если игрок знает тип умения, он сразу может вспомнить какие приёмы для него подходят. Не нужно запоминать правила работы с каждым умением в игре. Достаточно помнить правила только для пяти типов умений.
...
**Первый признак** — это отклонение от базовой схемы анимации. Для простоты мы рассматриваем только одно возможное отклонение: отсутствие фазы windup. Второе отклонение с заменой какой-то фазы на channel не существенно для приёмов работы с анимацией.
...
Обратите внимание, что чемпион может двигаться во время анимации firing некоторых статических умений. Пример: Darius Q. В этом случае причина движения — команда игрока move, а не анимация firing умения. Если игрок не будет давать команду двигаться, чемпион проиграет всю анимацию умения на одном месте.
