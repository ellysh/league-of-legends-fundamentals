...
В точке A игрок даёт команду применить умение. С этого момента чемпион проигрывает фазу анимации windup: натягивает тетиву лука. Она всегда длится 0.25 секунды для умения W.

В точке B анимация windup заканчивается. После этого чемпион проигрывает фазу firing: выпускает залп стрел. Она длится примерно 0.1 секунду.

В точке C чемпион начинает фазу recovery: достаёт из колчана следующую стрелу. Она длится примерно 1.65 секунды.

В точке D чемпион заканчивает полный цикл анимации. Прошло 2 секунды с момента, когда игрок дал команду в точке A.
...
Механика сброса таймера автоатаки заключается в следующем. Когда игрок применяет умение, текущий таймер АА сбрасывается. Это означает, что чемпион сразу может выполнить следующую атаку. Если в момент применения умения таймер АА не запущен, свойство не даёт никакого эффекта. Поэтому чтобы его реализовать, надо запускать таймер автоатаки каждый раз перед применением умения.

Рассмотрим, как построить комбо на сбросе таймера автоатаки. Для примера возьмём **комбо AA-E чемпиона Vi**. Её умение E имеет оба рассмотренных свойства: attack reset и uncancellable windup.
...
1. В точке A игрок нажимает правой кнопкой мыши на цель. С этого момента чемпион проигрывает фазу анимации windup: замахивается кулаком для удара. Она длится 22.5% от времени перезарядки АА — 1.55 секунды. Это равно примерно 0.35 секунды, согласно следующему расчёту:
...
3. В точке C игрок нажимает кнопку E и даёт команду на атаку цели. Тогда умение E отменяет анимацию AA recovery и сбрасывает таймер перезарядки атаки. После этого чемпион выполняет следующую атаку. Её фаза windup длится 0.35.

4. В точке D начинается анимация E firing.  Она длится 0.25 секунд — столько же, сколько для обычной автоатаки.

5. В точке E чемпиона начинает фазу E recovery. Она длится 0.95 секунд, как для автоатаки.

Анимацию E recovery на отрезке E-G можно отменить следующим действием:
...
Автоатака для этого не подходит. Её таймер перезарядки начинается в точке C. Время перезарядки равно 1.55 секунды. Отрезок C-E равен всего 0.6 секунды. Поэтому чемпион сможет выполнить следующую атаку только через 0.95 секунды после точки E.
