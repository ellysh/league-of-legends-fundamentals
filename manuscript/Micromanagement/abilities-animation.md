...
1. В точке A игрок нажимает кнопку Q. С этого момента чемпион проигрывает фазу анимации Q windup: замахивается топором. Она всегда длится 0.75 секунд.
...
3. В точке C начинается анимация фазы Q firing. В этот момент все противники в области поражения умения получают урон. Эта фаза длится примерно 0.4 секунды.
...
При исполнении комбо Q-F противник видит анимацию умения заранее. Но у него всё равно мало времени, чтобы отреагировать. Если он нажмёт flash слишком рано на отрезке A-B, то Darius не сделает flash. Противник может уклониться от удара, только если применит flash на отрезке B-C. Поэтому чем короче этот отрезок, тем качественнее исполнено комбо.

Вот общее правило, когда надо нажимать flash в комбо со сменой позиции:

> Если позиция меняется для нападения, flash надо нажимать как можно позже. Это не даст противнику времени на реакцию. Если позиция меняется для отступления, flash надо нажимать как можно раньше. Тогда чемпион раньше окажется на безопасной дистанции.
