...
Рассмотрим умения с такими фазами в качестве базового шаблона. Некоторые умения отличаются от этой схемы. Будем рассматривать их как отклонение от шаблона.

Начнём с простых умений, анимация которых по структуре такая же как у базовой атаки. Иллюстрация 3-7 демонстрирует временную диаграмму такого умения — Ashe W.
...
В точке C чемпион начинает фазу recovery: достаёт из колчана следующую стрелу. Она длится примерно 1.67 секунды.
...
Мы рассмотрели базовый шаблон анимации умений. От него есть два основных отклонения:
...
Разные типы умений позволяют отменять или изменять анимацию каждой фазы: windup, firing или recovery. Общего правила здесь нет. Всё зависит от конкретного умения и чемпиона.
