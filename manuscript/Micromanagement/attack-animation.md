...
2. В точке B анимация windup заканчивается. После этого чемпион проигрывает фазу firing: выпускает стрелу в цель. Она длится всего 3.37% от времени перезарядки автоатаки. Это равно 0.05 секунд:
{line-numbers: false, format: text}
```
